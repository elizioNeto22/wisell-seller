...
import CustomerSearchList from './ListaFornecedor'
import ListaFornecedor from './ListaFornecedor'

const PesquisarFornecedor = ({ entidadeId = 5, setOpenSearchForm }) => {
  ...
  return (
    <>
      <Form.Group className="page-header">
        <h4 className="page-title">Pesquisar fornecedores</h4>
      ...
      {showList && termo.length >= 3 && (
        <Form.Group>
          <ListaFornecedor entidade={entidadeId} termo={termo} onSelectFornecedor={onSelectFornecedor} />
        </Form.Group>
      )}
    </>
  )
